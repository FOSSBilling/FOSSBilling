...
test('API response factory maps authentication errors to unauthorized responses', function (): void {
    $response = (new ApiResponseFactory())->create(null, new FOSSBilling\Core\Exception\BaseException('Authentication Failed', null, 201));
...
test('API response factory preserves legacy ok status for unmapped application errors', function (): void {
    $response = (new ApiResponseFactory())->create(null, new FOSSBilling\Core\Exception\BaseException('Unexpected API error', null, 9999));
