...
function createSession(?SymfonySession $httpSession = null): FOSSBilling\Core\Security\Session
{
    return new FOSSBilling\Core\Security\Session($httpSession ?? new SymfonySession(new MockArraySessionStorage('PHPSESSID')));
}

/**
 * @return array{FOSSBilling\Core\Security\Session, Pimple\Container}
 */
...
test('session validation initializes a missing creation time', function (): void {
    ...
    $fingerprint = json_encode((new FOSSBilling\Core\Security\Fingerprint($di['request']))->fingerprint(), JSON_THROW_ON_ERROR);
...
test('disabled fingerprinting skips validation and stores an empty fingerprint', function (): void {
    $previousSetting = FOSSBilling\Core\System\Config::getProperty('security.perform_session_fingerprinting', true);
    FOSSBilling\Core\System\Config::setProperty('security.perform_session_fingerprinting', false, false);
    ...
    try {
        ...
        FOSSBilling\Core\System\Config::setProperty('security.perform_session_fingerprinting', $previousSetting, false);
    }
});
...
test('destroying a client login regenerates the session with the configured grace period, not zero', function (): void {
    ...
    $session = Mockery::mock(FOSSBilling\Core\Security\Session::class, [$httpSession])->makePartial();
