...
use FOSSBilling\Core\Security\RateLimitResult;
use Symfony\Component\Cache\Adapter\ArrayAdapter;
use Symfony\Component\HttpFoundation\Request;

function createRateLimiter(string $requestIp, array $whitelist = [], ?bool $enabled = null): FOSSBilling\Core\Security\RateLimiter
{
    ...
    $limiter = new class($whitelist, $enabled) extends FOSSBilling\Core\Security\RateLimiter {
...
test('consume or throw raises rate limit exception', function (): void {
    ...
    expect(fn (): RateLimitResult => $limiter->consumeOrThrow('client_password_reset_email', $subject))
        ->toThrow(function (FOSSBilling\Core\Security\RateLimitException $exception): void {
...
test('unknown policy throws exception', function (): void {
    ...
    expect(fn (): RateLimitResult => $limiter->consume('unknown_policy', 'subject'))
        ->toThrow(FOSSBilling\Core\Exception\BaseException::class, 'Rate limiter policy unknown_policy is not defined or invalid');
});
