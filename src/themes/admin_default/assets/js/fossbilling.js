   ...
   document.addEventListener("DOMContentLoaded", function() {
     ...
     if (tabParam) {
       ...
       if (tabElement) {
         ...
         const tabTrigger = document.querySelector(`a[href="#tab-${tabParam}"]`);
         if (tabTrigger) {
     ...
     //===== URL parameter support for order forms =====//
     // Handle period parameter for billing cycle selection
     ...
     // Handle domain name parameter (for domain orders)
     const domainNameParam = urlParams.get('name');
     if (domainNameParam) {
       const domainNameInput = document.querySelector('input[name*="[register_sld]"], input[name*="[transfer_sld]"], input[name*="[domain_sld]"]');
       if (domainNameInput) {
         domainNameInput.value = domainNameParam;
       }
     }

     // Handle TLD parameter (for domain orders)
     const tldParam = urlParams.get('tld');
     if (tldParam) {
       const tldInput = document.querySelector('select[name*="[register_tld]"], select[name*="[transfer_tld]"]');
       if (tldInput) {
         for (let i = 0; i < tldInput.options.length; i++) {
           if (tldInput.options[i].value === tldParam || tldInput.options[i].value === '.' + tldParam) {
             tldInput.selectedIndex = i;
             break;
           }
         }
       }
