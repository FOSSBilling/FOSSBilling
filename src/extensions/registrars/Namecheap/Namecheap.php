...
class Namecheap extends \FOSSBilling\Extension\Contract\Registrar\AdapterAbstract
{
    ...
    public function __construct($options)
    {
        if (!empty($options['api-key'])) {
            ...
            throw new \FOSSBilling\Extension\Contract\Registrar\Exception('The ":domain_registrar" domain registrar is not fully configured. Please configure the :missing', [':domain_registrar' => 'Namecheap', ':missing' => 'API Key'], 3001);
        }

        if (!empty($options['username'])) {
            $this->config['username'] = $options['username'];
        } else {
            throw new \FOSSBilling\Extension\Contract\Registrar\Exception('The ":domain_registrar" domain registrar is not fully configured. Please configure the :missing', [':domain_registrar' => 'Namecheap', ':missing' => 'Username'], 3001);
        }
        ...
        if (!empty($options['ip'])) {
            ...
            throw new \FOSSBilling\Extension\Contract\Registrar\Exception('The ":domain_registrar" domain registrar is not fully configured. Please configure the :missing', [':domain_registrar' => 'Namecheap', ':missing' => 'server IP address'], 3001);
        }
    }
     ...
     * If domain is not registered should throw \FOSSBilling\Extension\Contract\Registrar\Exception.
     *
     * @throws \FOSSBilling\Extension\Contract\Registrar\Exception
     */
    public function getDomainDetails(\FOSSBilling\Extension\Contract\Registrar\Domain $domain): \FOSSBilling\Extension\Contract\Registrar\Domain
    {
        $params = [
            'DomainName' => $domain->getName(),
        ...
        $newDomain = new \FOSSBilling\Extension\Contract\Registrar\Domain();
        ...
        $techContact = new \FOSSBilling\Extension\Contract\Registrar\Domain\Contact();
        $billingContact = new \FOSSBilling\Extension\Contract\Registrar\Domain\Contact();
     ...
     * @throws \FOSSBilling\Extension\Contract\Registrar\Exception
     */
    public function getEpp(\FOSSBilling\Extension\Contract\Registrar\Domain $domain): never
    {
        throw new \FOSSBilling\Extension\Contract\Registrar\Exception(':type: does not support :action:', [':type:' => 'Namecheap', ':action:' => __trans('retrieving the transfer code')]);
    }
     ...
     * @throws \FOSSBilling\Extension\Contract\Registrar\Exception
     */
    public function deleteDomain(\FOSSBilling\Extension\Contract\Registrar\Domain $domain): never
    {
        throw new \FOSSBilling\Extension\Contract\Registrar\Exception(':type: does not support :action:', [':type:' => 'Namecheap', ':action:' => __trans('deleting domains')]);
    }
     ...
     * @throws \FOSSBilling\Extension\Contract\Registrar\Exception
     */
    public function enablePrivacyProtection(\FOSSBilling\Extension\Contract\Registrar\Domain $domain)
    {
     ...
     * @throws \FOSSBilling\Extension\Contract\Registrar\Exception
     */
    public function disablePrivacyProtection(\FOSSBilling\Extension\Contract\Registrar\Domain $domain)
    {
        $privacyInfo = $this->getPrivacyInfo($domain);
     ...
     * @throws \FOSSBilling\Extension\Contract\Registrar\Exception
     */
    public function getLock(\FOSSBilling\Extension\Contract\Registrar\Domain $domain): bool
    {
        $params = [
            'DomainName' => $domain->getName(),
