...
namespace FOSSBilling\Extension\Registrar\Internetbs;

use Symfony\Contracts\HttpClient\Exception\HttpExceptionInterface;

class Internetbs extends \FOSSBilling\Extension\Contract\Registrar\AdapterAbstract
{
    public $config = [
        'apikey' => null,
        'password' => null,
    ...
    public function __construct($options)
    {
        if (isset($options['apikey']) && !empty($options['apikey'])) {
            ...
            throw new \FOSSBilling\Extension\Contract\Registrar\Exception('The ":domain_registrar" domain registrar is not fully configured. Please configure the :missing', [':domain_registrar' => 'Internetbs', ':missing' => 'Internetbs API key'], 3001);
        }
        ...
        if (isset($options['password']) && !empty($options['password'])) {
            ...
            throw new \FOSSBilling\Extension\Contract\Registrar\Exception('The ":domain_registrar" domain registrar is not fully configured. Please configure the :missing', [':domain_registrar' => 'Internetbs', ':missing' => 'Internetbs API password'], 3001);
        }
    }
    ...
    {
        throw new \FOSSBilling\Extension\Contract\Registrar\Exception(':type: does not support :action:', [':type:' => 'Internet.bs', ':action:' => __trans('deleting domains')]);
    }
    ...
    private function _parseResult($data): array
    {
        ...
        if (array_key_exists('status', $result)
            ...
            throw new \FOSSBilling\Extension\Contract\Registrar\Exception($result['message']);
        }
     ...
     *
     * @return \FOSSBilling\Extension\Contract\Registrar\Domain
     */
    private function _createDomainObj($result, \FOSSBilling\Extension\Contract\Registrar\Domain $domain)
    {
