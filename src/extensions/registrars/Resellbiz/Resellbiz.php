...
namespace FOSSBilling\Extension\Registrar\Resellbiz;

class Resellbiz extends \FOSSBilling\Extension\Registrar\Resellerclub\Resellerclub
{
    public function __construct($options)
    {
        if (isset($options['userid']) && !empty($options['userid'])) {
            ...
            throw new \FOSSBilling\Extension\Contract\Registrar\Exception('The ":domain_registrar" domain registrar is not fully configured. Please configure the :missing', [':domain_registrar' => 'Resell.biz', ':missing' => 'Resell.biz Reseller ID'], 3001);
        }
        ...
        if (isset($options['api-key']) && !empty($options['api-key'])) {
            ...
            throw new \FOSSBilling\Extension\Contract\Registrar\Exception('The ":domain_registrar" domain registrar is not fully configured. Please configure the :missing', [':domain_registrar' => 'Resell.biz', ':missing' => 'Resell.biz API Key'], 3001);
        }
    }
