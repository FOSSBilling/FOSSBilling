...
namespace FOSSBilling\Extension\Registrar\Netearthone;

class Netearthone extends \FOSSBilling\Extension\Registrar\Resellerclub\Resellerclub
{
    public function __construct($options)
    {
        if (isset($options['userid']) && !empty($options['userid'])) {
            ...
            throw new \FOSSBilling\Extension\Contract\Registrar\Exception('The ":domain_registrar" domain registrar is not fully configured. Please configure the :missing', [':domain_registrar' => 'NetEarthOne', ':missing' => 'NetEarthOne Reseller ID'], 3001);
        }
        ...
        if (isset($options['api-key']) && !empty($options['api-key'])) {
            ...
            throw new \FOSSBilling\Extension\Contract\Registrar\Exception('The ":domain_registrar" domain registrar is not fully configured. Please configure the :missing', [':domain_registrar' => 'NetEarthOne', ':missing' => 'NetEarthOne API Key'], 3001);
        }
    }
