...
namespace FOSSBilling\Extension\Registrar\Resellerclub;
...
class Resellerclub extends \FOSSBilling\Extension\Contract\Registrar\AdapterAbstract
{
    public $config = [
        'userid' => null,
        'password' => null,
    ...
    public function __construct($options)
    {
        if (isset($options['userid']) && !empty($options['userid'])) {
            ...
            throw new \FOSSBilling\Extension\Contract\Registrar\Exception('The ":domain_registrar" domain registrar is not fully configured. Please configure the :missing', [':domain_registrar' => 'ResellerClub', ':missing' => 'ResellerClub Reseller ID'], 3001);
        }
        ...
        if (isset($options['api-key']) && !empty($options['api-key'])) {
            ...
            throw new \FOSSBilling\Extension\Contract\Registrar\Exception('The ":domain_registrar" domain registrar is not fully configured. Please configure the :missing', [':domain_registrar' => 'ResellerClub', ':missing' => 'ResellerClub API Key'], 3001);
        }
    }
    ...
    public function isDomainAvailable(\FOSSBilling\Extension\Contract\Registrar\Domain $domain): bool
    {
        $params = [
    ...
    private function _getDomainOrderId(\FOSSBilling\Extension\Contract\Registrar\Domain $d)
    {
        $required_params = [
    ...
    public function getDomainDetails(\FOSSBilling\Extension\Contract\Registrar\Domain $d)
    {
        ...
        $c = new \FOSSBilling\Extension\Contract\Registrar\Domain\Contact();
    ...
    public function registerDomain(\FOSSBilling\Extension\Contract\Registrar\Domain $domain)
    {
        ...
        if ($tld == '.au' || $tld == '.net.au' || $tld == '.com.au') {
            ...
            if (strlen(trim($contact->getCompanyNumber())) == 0) {
                throw new \FOSSBilling\Extension\Contract\Registrar\Exception('A valid contact company number is mandatory for registering an .AU domain name');
            }
    ...
    public function getEpp(\FOSSBilling\Extension\Contract\Registrar\Domain $domain)
    {
        $params = [
            'order-id' => $this->_getDomainOrderId($domain),
    ...
    private function _getCustomerDetails(\FOSSBilling\Extension\Contract\Registrar\Domain $domain): array
    {
        $c = $domain->getContactRegistrar();
        $params = [
            'username' => $c->getEmail(),
        ...
        } catch (\FOSSBilling\Extension\Contract\Registrar\Exception) {
    ...
    private function _createCustomer(\FOSSBilling\Extension\Contract\Registrar\Domain $domain)
    {
        $c = $domain->getContactRegistrar();
        $company = $c->getCompany();
    ...
    public function getContactIdForDomain(\FOSSBilling\Extension\Contract\Registrar\Domain $domain)
    {
        $c = $domain->getContactRegistrar();
        $customer = $this->_getCustomerDetails($domain);
        $customer_id = $customer['customerid'];
    ...
    private function _getDefaultContactDetails(\FOSSBilling\Extension\Contract\Registrar\Domain $domain, $customerid)
    {
        $params = [
    ...
    private function _hasCompletedOrder(\FOSSBilling\Extension\Contract\Registrar\Domain $domain)
    {
        try {
    ...
    protected function _makeRequest($url, $params = [], $method = 'GET', $type = 'json'): array|string
    {
        ...
        try {
            ...
            $e = new \FOSSBilling\Extension\Contract\Registrar\Exception("HttpClientException: {$error->getMessage()}.");
    ...
    private function _getAllContacts($tld, $customer_id, \FOSSBilling\Extension\Contract\Registrar\Domain\Contact $client): array
    {
        if ($tld[0] != '.') {
        ...
        if ($tld == '.es') {
            if (strlen(trim((string) $client->getDocumentNr())) == 0) {
                throw new \FOSSBilling\Extension\Contract\Registrar\Exception('A document number is required while registering an ES domain name. Enable a custom client field with a title containing "passport" or "document" (e.g. "Passport Number") and ask the client to fill it in.');
            }
        ...
        if ($tld == '.asia') {
            if (strlen(trim((string) $client->getDocumentNr())) == 0) {
                throw new \FOSSBilling\Extension\Contract\Registrar\Exception('A document number is required while registering an ASIA domain name. Enable a custom client field with a title containing "passport" or "document" (e.g. "Passport Number") and ask the client to fill it in.');
            }
        ...
        if ($tld == '.ru' || $tld == '.com.ru' || $tld == '.org.ru' || $tld == '.net.ru') {
            if (strlen(trim($client->getBirthday())) === 0 || strtotime($client->getBirthday()) === false) {
                throw new \FOSSBilling\Extension\Contract\Registrar\Exception('Valid contact birthdate is required while registering RU domain name');
            }

            if (strlen(trim((string) $client->getDocumentNr())) === 0) {
                throw new \FOSSBilling\Extension\Contract\Registrar\Exception('A document number is required while registering a RU domain name. Enable a custom client field with a title containing "passport" or "document" (e.g. "Passport Number") and ask the client to fill it in.');
            }
    ...
    private function _getContact($contact, $customer_id, $type = 'Contact')
    {
        try {
            ...
            if ($result['recsonpage'] < 1) {
                throw new \FOSSBilling\Extension\Contract\Registrar\Exception('Contact not found');
            }
            $existing_contact_id = $result['result'][0]['entity.entityid'];
            $this->_makeRequest('contacts/delete', ['contact-id' => $existing_contact_id], 'POST');
        } catch (\FOSSBilling\Extension\Contract\Registrar\Exception $e) {
