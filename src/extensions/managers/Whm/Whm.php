...
namespace FOSSBilling\Extension\Manager\Whm;
 ...
 */
class Whm extends \FOSSBilling\Extension\Contract\Server\Manager
{
    /**
     ...
     *
     * @throws \FOSSBilling\Extension\Contract\Server\Exception if any necessary configuration options are missing
     */
    public function init(): void
    {
        if (empty($this->_config['host'])) {
            throw new \FOSSBilling\Extension\Contract\Server\Exception('The ":server_manager" server manager is not fully configured. Please configure the :missing', [':server_manager' => 'cPanel WHM', ':missing' => 'hostname'], 2001);
        }

        if (empty($this->_config['username'])) {
            throw new \FOSSBilling\Extension\Contract\Server\Exception('The ":server_manager" server manager is not fully configured. Please configure the :missing', [':server_manager' => 'cPanel WHM', ':missing' => 'username'], 2001);
        }

        if (empty($this->_config['password']) && empty($this->_config['accesshash'])) {
            throw new \FOSSBilling\Extension\Contract\Server\Exception('The ":server_manager" server manager is not fully configured. Please configure the :missing', [':server_manager' => 'cPanel WHM', ':missing' => 'authentication credentials'], 2001);
        }
     ...
     *
     * @param \FOSSBilling\Extension\Contract\Server\Account|null $account The account for which to get the login URL. This parameter is currently not used.
     *
     ...
     * Sends a request to the WHM server to get the account's details and updates the \FOSSBilling\Extension\Contract\Server\Account object accordingly.
     *
     * @param \FOSSBilling\Extension\Contract\Server\Account $account the account to be synchronized
     *
     * @return \FOSSBilling\Extension\Contract\Server\Account the updated account
     *
     * @throws \FOSSBilling\Extension\Contract\Server\Exception if an error occurs during the request, or if the account does not exist on the WHM server
     */
     ...
     *
     * @throws \FOSSBilling\Extension\Contract\Server\Exception if an error occurs during the request, or if the response from the WHM server indicates an error
     */
     ...
     *
     * @param \FOSSBilling\Extension\Contract\Server\Account $account the account for which to change the package
     ...
     *
     * @param \FOSSBilling\Extension\Contract\Server\Account $account     the account for which to change the password
    ...
    public function changeAccountPassword(\FOSSBilling\Extension\Contract\Server\Account $account, string $newPassword): bool
    {
        ...
        if (isset($result->passwd[0]) && $result->passwd[0]->status == 0) {
            throw new \FOSSBilling\Extension\Contract\Server\Exception($result->passwd[0]->statusmsg);
        }
     ...
     *
     * @param \FOSSBilling\Extension\Contract\Server\Account $account     the account for which to change the username
     ...
     * @throws \FOSSBilling\Extension\Contract\Server\Exception if an error occurs during the request
     */
    public function changeAccountUsername(\FOSSBilling\Extension\Contract\Server\Account $account, string $newUsername): bool
    {
     ...
     *
     * @param \FOSSBilling\Extension\Contract\Server\Account $account   the account for which to change the domain
     ...
     *
     * @param \FOSSBilling\Extension\Contract\Server\Account $account the account for which to change the IP
     ...
     * @throws \FOSSBilling\Extension\Contract\Server\Exception if an error occurs during the request
     */
    public function changeAccountIp(\FOSSBilling\Extension\Contract\Server\Account $account, string $newIp): bool
    {
        // Log the IP change
     ...
     * logging them and throwing a \FOSSBilling\Extension\Contract\Server\Exception if necessary.
     *
    ...
    private function request(string $action, array $params = []): mixed
    {
        ...
        try {
            ...
            $e = new \FOSSBilling\Extension\Contract\Server\Exception('HttpClientException: :error', [':error' => $error->getMessage()]);
        ...
        // Check the response for errors and throw a \FOSSBilling\Extension\Contract\Server\Exception if any are found
        if (!is_object($json)) {
     ...
     *
     * @param \FOSSBilling\Extension\Contract\Server\Package $package the package to check
     * @param bool                                           $create  whether to create the package if it does not exist
     *
     * @throws \FOSSBilling\Extension\Contract\Server\Exception if an error occurs during the request
     */
    private function checkPackageExists(\FOSSBilling\Extension\Contract\Server\Package $package, bool $create = false): void
    {
     ...
     *
     * @param \FOSSBilling\Extension\Contract\Server\Package $package the package for which to generate a name
     *
