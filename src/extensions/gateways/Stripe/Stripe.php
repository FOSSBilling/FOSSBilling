...
class Stripe implements \FOSSBilling\InjectionAwareInterface
{
    ...
    public function __construct(private $config)
    {
        if ($this->config['test_mode']) {
            if (!isset($this->config['test_api_key'])) {
                throw new \FOSSBilling\Extension\Contract\Payment\Exception('The ":pay_gateway" payment gateway is not fully configured. Please configure the :missing', [':pay_gateway' => 'Stripe', ':missing' => 'Test API Key'], 4001);
            }
            if (!isset($this->config['test_pub_key'])) {
                throw new \FOSSBilling\Extension\Contract\Payment\Exception('The ":pay_gateway" payment gateway is not fully configured. Please configure the :missing', [':pay_gateway' => 'Stripe', ':missing' => 'Test publishable key'], 4001);
            }

            $this->stripe = new StripeClient($this->config['test_api_key']);
        } else {
            if (!isset($this->config['api_key'])) {
                throw new \FOSSBilling\Extension\Contract\Payment\Exception('The ":pay_gateway" payment gateway is not fully configured. Please configure the :missing', [':pay_gateway' => 'Stripe', ':missing' => 'API key'], 4001);
            }
            if (!isset($this->config['pub_key'])) {
                throw new \FOSSBilling\Extension\Contract\Payment\Exception('The ":pay_gateway" payment gateway is not fully configured. Please configure the :missing', [':pay_gateway' => 'Stripe', ':missing' => 'Publishable key'], 4001);
            }
    ...
    private function getStripeRecurringParams(string $periodCode): array
    {
        ...
        if ($intervalCount > $maxIntervalCount) {
            throw new \FOSSBilling\Extension\Contract\Payment\Exception('The billing period ":period" is not supported by the Stripe payment gateway. The maximum supported interval is :max :interval(s)', [':period' => $periodCode, ':max' => $maxIntervalCount, ':interval' => $interval]);
        }
