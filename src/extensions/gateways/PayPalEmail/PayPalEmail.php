...
namespace FOSSBilling\Extension\Gateway\PayPalEmail;

use FOSSBilling\Environment;
use Pimple\Container;

class PayPalEmail extends \FOSSBilling\Extension\Contract\Payment\AdapterAbstract implements \FOSSBilling\InjectionAwareInterface
{
    ...
    public function __construct(private $config)
    {
        if (!isset($this->config['email'])) {
            throw new \FOSSBilling\Extension\Contract\Payment\Exception('The ":pay_gateway" payment gateway is not fully configured. Please configure the :missing', [':pay_gateway' => 'PayPal', ':missing' => 'PayPal Email address'], 4001);
        }
    }
    ...
    public function processTransaction($api_admin, $id, $data, $gateway_id): void
    {
        if (!Environment::isTesting() && !$this->_isIpnValid($data)) {
            throw new \FOSSBilling\Extension\Contract\Payment\Exception('IPN is invalid');
        }
        ...
        switch ($txnType) {
            ...
            case 'subscr_payment':
                ...
                if ($this->isIpnDuplicate($ipn)) {
                    throw new \FOSSBilling\Extension\Contract\Payment\Exception('Cannot process duplicate IPN');
                }
