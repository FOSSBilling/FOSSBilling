 ...
 *                       just PayGatewayProduct created
 */
function payGatewayProductEntityManager(): EntityManager
{
    ...
    $metadata = [$entityManager->getClassMetadata(PayGatewayProduct::class)];
...
test('findOneByGatewayAndCacheKey finds the cached product/price for that gateway and key', function (): void {
    ...
    $product->setPayGatewayId(1);
    $product->setCacheKey(hash('sha256', 'Basic Hosting|usd|999|month'));
    ...
    $found = $em->getRepository(PayGatewayProduct::class)
        ->findOneByGatewayAndCacheKey(1, hash('sha256', 'Basic Hosting|usd|999|month'));
...
test('findOneByGatewayAndCacheKey returns null when no row matches', function (): void {
    ...
    expect($em->getRepository(PayGatewayProduct::class)->findOneByGatewayAndCacheKey(1, str_repeat('0', 64)))->toBeNull();
});
...
test('a second product row for the same gateway and cache key violates the unique constraint', function (): void {
    ...
    $cacheKey = hash('sha256', 'Basic Hosting|usd|999|month');
