...
class Service
{
    ...
    public function getPage($id, $type = 'id'): ?array
    {
        ...
        if (!in_array($type, $allowedColumns, true)) {
            throw new \FOSSBilling\Exception\BaseException('Invalid column type: :type', [':type' => $type]);
        }
    ...
    public function createPage($title, $description, $keywords, $content): int
    {
        ...
        throw new \FOSSBilling\Exception\BaseException('Unable to generate a unique slug for the custom page.');
    }
