...
class Admin extends \FOSSBilling\Core\Api\AbstractApi
{
    ...
    public function create_form($data)
    {
        ...
        if (isset($data['type']) && !in_array(strtolower($data['type']), ['horizontal', 'default'])) {
            throw new \FOSSBilling\Core\Exception\BaseException('Form style was not found in predefined list', null, 3657);
        }
    ...
    public function add_field($data)
    {
        ...
        if (!isset($data['type']) || !$service->isValidFieldType($data['type'])) {
            throw new \FOSSBilling\Core\Exception\BaseException('Form field type is invalid', null, 2684);
        }
        if (!isset($data['form_id'])) {
            throw new \FOSSBilling\Core\Exception\InformationException('Form id was not passed', null, 1822);
        }
        if (isset($data['options']) && is_array($data['options']) && !$service->isArrayUnique($data['options'])) {
            throw new \FOSSBilling\Core\Exception\InformationException('This input type must have unique values', null, 3658);
        }
    ...
    public function update_form_settings($data)
    {
        ...
        if ($type !== 'horizontal' && $type !== 'default') {
            throw new \FOSSBilling\Core\Exception\BaseException('Field type not supported', null, 3207);
        }
