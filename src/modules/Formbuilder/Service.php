...
class Service implements InjectionAwareInterface
{
    ...
    private function slugify($text): string
    {
        ...
        if (is_numeric(substr((string) $text, 0, 1))) {
            throw new \FOSSBilling\Exception\InformationException('Field name cannot start with number.', null, 1649);
        }

        if (empty($text)) {
            throw new \FOSSBilling\Exception\InformationException('Field name cannot be empty. Please make sure it is not empty and does not contain special characters.', null, 3502);
        }
    ...
    public function updateField(array $field): int
    {
        ...
        if ($this->formFieldNameExists([
            ...
            throw new \FOSSBilling\Exception\InformationException('Unfortunately field with this name exists in this form already. Form must have different field names.', null, 7628);
        }
        ...
        if (isset($field['type'])) {
            if ($field['type'] == 'checkbox' || $field['type'] == 'radio' || $field['type'] == 'select') {
                if (!$this->isArrayUnique(array_filter($field['values'], fn ($v): bool => strlen((string) $v) > 0))) {
                    throw new \FOSSBilling\Exception\InformationException(ucfirst($field['type']) . ' values must be unique', null, 1597);
                }

                if (!$this->isArrayUnique(array_filter($field['labels'], fn ($v): bool => strlen((string) $v) > 0))) {
                    throw new \FOSSBilling\Exception\InformationException(ucfirst($field['type']) . ' labels must be unique', null, 1598);
                }
    ...
    public function getForm(int $formId): array
    {
        ...
        if ($result === false) {
            throw new \FOSSBilling\Exception\InformationException('Form was not found');
        }
    ...
    public function getField($fieldId)
    {
        ...
        if ($result === false) {
            throw new \FOSSBilling\Exception\InformationException('Field was not found');
        }
