...
class Service implements InjectionAwareInterface
{
    ...
    public function addItem(\Model_Cart $cart, \Model_Product $product, array $data)
    {
        ...
        if ($this->isRecurrentPricing($product)) {
            ...
            if (!$this->isPeriodEnabledForProduct($product, $data['period'])) {
                throw new \FOSSBilling\Exception('Selected billing period is invalid');
            }
        }
        ...
        if (!$this->isStockAvailable($product, $qty)) {
            throw new \FOSSBilling\Exception('This item is currently out of stock');
        }
                ...
                if ($addon instanceof \Model_Product) {
                    if ($this->isRecurrentPricing($addon)) {
                        ...
                        if (!$this->isPeriodEnabledForProduct($addon, $ac['period'])) {
                            throw new \FOSSBilling\Exception('Selected billing period is invalid for the selected addon');
                        }
                    }
    ...
    public function applyPromo(\Model_Cart $cart, \Model_Promo $promo)
    {
        ...
        if ($this->isEmptyCart($cart)) {
            throw new \FOSSBilling\Exception('Add products to cart before applying promo code');
        }
    ...
    public function checkoutCart(\Model_Cart $cart, \Model_Client $client, $gateway_id = null)
    {
        if ($cart->promo_id) {
            ...
            if (!$this->isClientAbleToUsePromo($client, $promo)) {
                throw new \FOSSBilling\Exception('You have already used this promo code. Please remove promo code and checkout again.', null, 9874);
            }

            if (!$promo instanceof \Model_Promo) {
                throw new \FOSSBilling\Exception('Promo code is expired or does not exist');
            }

            if (!$this->isPromoAvailableForClientGroup($promo)) {
                throw new \FOSSBilling\Exception('Promo can not be applied to your account');
            }
        }
    ...
    public function createFromCart(\Model_Client $client, $gateway_id = null)
    {
        ...
        if ((is_countable($ca['items']) ? count($ca['items']) : 0) == 0) {
            throw new \FOSSBilling\Exception('Can not checkout an empty cart');
        }
        ...
        if ($client->currency != $currency->code) {
            throw new \FOSSBilling\Exception('Selected currency :selected does not match your profile currency :code. Please change cart currency to continue.', [':selected' => $currency->code, ':code' => $client->currency]);
        }
        ...
        foreach ($this->getCartProducts($cart) as $p) {
            ...
            if (is_null($product) || $product->status !== 'enabled') {
                throw new \FOSSBilling\Exception('Unable to complete order. One or more of the selected products are invalid.');
            }
