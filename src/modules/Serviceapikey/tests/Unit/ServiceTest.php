...
function serviceApiKeyBuildOrderMock(string $status, ?string $expiresAt): Model_ClientOrder
{
...
test('isActive returns true for active order with future expires_at', function (): void {
    ...
    $order = serviceApiKeyBuildOrderMock('active', date('Y-m-d H:i:s', time() + 86400));
...
test('isActive returns false for expired order', function (): void {
    ...
    $expiredOrder = serviceApiKeyBuildOrderMock('active', date('Y-m-d H:i:s', time() - 3600));
    $model = serviceApiKeyBuildModel(7);
...
test('isActive returns false for inactive order status', function (): void {
    ...
    $suspendedOrder = serviceApiKeyBuildOrderMock(Model_ClientOrder::STATUS_SUSPENDED, null);
    $model = serviceApiKeyBuildModel(9);

    $dbMock = Mockery::mock(Box_Database::class);
    $dbMock->shouldReceive('findOne')->once()->andReturn($suspendedOrder);
...
test('isActive returns true for active order with null expires_at', function (): void {
    ...
    $oneTimeOrder = serviceApiKeyBuildOrderMock('active', null);
    $model = serviceApiKeyBuildModel(11);

    $dbMock = Mockery::mock(Box_Database::class);
    $dbMock->shouldReceive('findOne')->once()->andReturn($oneTimeOrder);
