    ...
    public function save_theme_settings(\Box_App $app, $theme)
    {
        ...
        try {
            if (!$t->isAssetsPathWritable()) {
                throw new \FOSSBilling\Exception('Theme ":name" assets folder is not writable. Files cannot be uploaded and settings cannot be saved. Set folder permissions to 777', [':name' => $t->getName()]);
            }
