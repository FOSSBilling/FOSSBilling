...
class Service implements InjectionAwareInterface
{
    ...
    public function getExtensionsList($filter): array
    {
        ...
        foreach ($result as $key => $value) {
            ...
            if ($icon_url) {
                $iconPath = Path::join(SYSTEM_URL, $icon_url);
            }
        ...
        foreach ($result as $key => $value) {
            $icon_url = $value['icon_url'] ?? null;
            if ($icon_url) {
                $iconPath = Path::join(PATH_MODS, ucfirst($value['id']), Path::getFilenameWithoutExtension($icon_url));
                if ($this->filesystem->exists($iconPath)) {
                    $result[$key]['icon_path'] = 'mod_' . ucfirst($value['id']) . '_' . Path::getFilenameWithoutExtension($icon_url);
                }
            }
        }
    ...
    public function downloadAndExtract($type, $id)
    {
        ...
        $this->filesystem->mkdir($extractedPath, 0755);
