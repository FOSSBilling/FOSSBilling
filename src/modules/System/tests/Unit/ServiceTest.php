...
test('checkLimits does not throw when entity count is below the default limit', function (): void {
    $entityClass = Box\Mod\Product\Entity\ProductCategory::class;
    $service = new Service();
    ...
    $queryBuilder->shouldReceive('from')->once()->andReturn($queryBuilder);
    $queryBuilder->shouldReceive('getQuery')->once()->andReturn($query);
    $query->shouldReceive('getSingleScalarResult')->once()->andReturn('1');

    $emMock->shouldReceive('createQueryBuilder')->once()->andReturn($queryBuilder);

    $di = container();
    $di['em'] = $emMock;
    $service->setDi($di);

    $service->checkLimits($entityClass, 5);

    expect(true)->toBeTrue();
});

test('checkLimits throws InformationException when entity count meets or exceeds the default limit', function (): void {
    $entityClass = Box\Mod\Product\Entity\ProductCategory::class;
    $service = new Service();
    ...
    $queryBuilder->shouldReceive('from')->once()->andReturn($queryBuilder);
    $queryBuilder->shouldReceive('getQuery')->once()->andReturn($query);
    $query->shouldReceive('getSingleScalarResult')->once()->andReturn('2');
    ...
    expect(fn () => $service->checkLimits($entityClass, 2))
        ->toThrow(FOSSBilling\InformationException::class);
});

test('checkLimits honours configured override over the default limit', function (): void {
    $entityClass = Box\Mod\Product\Entity\ProductCategory::class;
    $service = new Service();
    ...
    $queryBuilder->shouldReceive('from')->once()->andReturn($queryBuilder);
    $queryBuilder->shouldReceive('getQuery')->once()->andReturn($query);
    // Override of 10 and count of 2, so the default of 2 is overridden and the
    // check should pass without throwing.
    $query->shouldReceive('getSingleScalarResult')->once()->andReturn('2');

    $emMock->shouldReceive('createQueryBuilder')->once()->andReturn($queryBuilder);

    $dbalResult = Mockery::mock(Doctrine\DBAL\Result::class);
    $dbalResult->shouldReceive('fetchOne')->once()->andReturn('10');
    $dbalQueryBuilder = Mockery::mock(Doctrine\DBAL\Query\QueryBuilder::class);
    foreach (['select', 'from', 'where', 'setParameter'] as $method) {
        $dbalQueryBuilder->shouldReceive($method)->byDefault()->andReturn($dbalQueryBuilder);
    }
    $dbalQueryBuilder->shouldReceive('executeQuery')->once()->andReturn($dbalResult);
    $dbal = Mockery::mock(Doctrine\DBAL\Connection::class);
    $dbal->shouldReceive('createQueryBuilder')->once()->andReturn($dbalQueryBuilder);
    ...
    $service->checkLimits($entityClass, 2);
