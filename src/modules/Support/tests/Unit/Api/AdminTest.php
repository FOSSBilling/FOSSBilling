...
test('batch ticket auto close', function (): void {
    ...
    $api->setService($serviceMock);
    $di = container();
    $di['logger'] = $this->createStub(FOSSBilling\Logging\Logger::class);
    $api->setDi($di);
...
test('kb article get list', function (): void {
    ...
    $pager->shouldReceive('paginateDoctrineQuery')
        ->once()
        ->with($qb, Mockery::type(FOSSBilling\Pagination\Options::class), Mockery::type('object'))
        ->andReturn(['list' => []]);
    $di['pager'] = $pager;
    $adminApi->setDi($di);
...
test('kb article get not found exception', function (): void {
    ...
    $adminApi->setDi($di);
    $adminApi->setService($kbService);
    expect(fn (): array => $adminApi->kb_article_get($data))->toThrow(FOSSBilling\Exception\BaseException::class);
});
...
test('kb article delete not found exception', function (): void {
    ...
    expect(fn (): bool => $adminApi->kb_article_delete(['id' => 1]))->toThrow(FOSSBilling\Exception\BaseException::class);
});
...
test('kb category get not found exception', function (): void {
    ...
    expect(fn (): array => $adminApi->kb_category_get($data))->toThrow(FOSSBilling\Exception\BaseException::class);
});
...
test('kb category update not found', function (): void {
    ...
    expect(fn (): bool => $adminApi->kb_category_update($data))->toThrow(FOSSBilling\Exception\BaseException::class);
});
...
test('kb category delete not found', function (): void {
    ...
    expect(fn (): bool => $adminApi->kb_category_delete($data))->toThrow(FOSSBilling\Exception\BaseException::class);
});
