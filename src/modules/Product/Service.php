...
class Service implements InjectionAwareInterface
{
    ...
    public function validateSelectedAddonsForProduct(Product $product, array $addons): void
    {
        ...
        foreach ($addons as $addonId => $properties) {
            ...
            if (!$addon instanceof Product || $addon->getStatus() !== 'enabled' || !in_array((int) $addonId, $validAddons)) {
                throw new \FOSSBilling\Core\Exception\InformationException('One or more of your selected add-ons are invalid for the associated product.');
            }
        }
    }
    ...
    public function getProductPricingArray(Product $product): array
    {
        ...
        throw new \FOSSBilling\Core\Exception\BaseException('Product pricing could not be determined.');
    }
    ...
    public function updateProduct(Product $model, $data): bool
    {
        ...
        if (isset($data['pricing'])) {
            ...
            if (!isset($data['pricing']['type']) || !array_key_exists($data['pricing']['type'], $types)) {
                throw new \FOSSBilling\Core\Exception\InformationException('Pricing type is required');
            }
    ...
    public function deleteProduct(Product $product): bool
    {
        ...
        if ($orderService->productHasOrders($product)) {
            throw new \FOSSBilling\Core\Exception\InformationException('Cannot remove product which has active orders.');
        }
        $id = $product->getId();
    ...
    public function removeProductCategory(ProductCategory $category): bool
    {
        if ($this->getProductRepository()->hasProductsInCategory((int) $category->getId())) {
            throw new \FOSSBilling\Core\Exception\InformationException('Cannot remove product category with products');
        }
        $id = $category->getId();
    ...
    public function assertUpgradeAllowedByIds(int $productId, int $upgradeProductId): void
    {
        ...
        throw new \FOSSBilling\Core\Exception\InformationException('Sorry, but ":product" is not allowed to be upgraded to ":upgrade"', [':product' => $product->getTitle() ?? 'unknown', ':upgrade' => $upgrade->getTitle() ?? 'unknown']);
    }
        ...
        foreach ($addons as $id => $addonConfig) {
            ...
            if ($this->isRecurrentProductPricing($addon)) {
                ...
                if (!$this->isProductPeriodEnabled($addon, (string) $addonConfig['period'])) {
                    throw new \FOSSBilling\Core\Exception\InformationException('Selected billing period is invalid for the selected add-on');
                }
            }
    ...
    public function reduceStock(Product|int $product, $qty): bool
    {
        ...
        if ($updated === 0) {
            throw new \FOSSBilling\Core\Exception\InformationException('Product :id is out of stock.', [':id' => $resolvedProduct->getId()], 831);
        }
    ...
    public function createPromo($code, $type, $value, $products, $periods, $clientGroups, $data): int
    {
        if ($this->getPromoRepository()->findOneBy(['code' => $code]) instanceof Promo) {
            throw new \FOSSBilling\Core\Exception\InformationException('This promotion code already exists.');
        }
    ...
    public function findPromoById(int $id): Promo
    {
        ...
        if (!$promo instanceof Promo) {
            throw new \FOSSBilling\Core\Exception\InformationException('Promo not found');
        }
    ...
    public function usePromo(Promo $promo): void
    {
        ...
        if ($affectedRows === 0) {
            throw new \FOSSBilling\Core\Exception\InformationException('This promo code has reached its maximum number of uses.');
        }
    }
    ...
    public function getRenewalPromoAdjustment(Order $order, float $price, float $quantity): ?array
    {
        ...
        if ($product->getType() === self::DOMAIN) {
            ...
            if ($rate === null) {
                throw new \FOSSBilling\Core\Exception\BaseException("Currency conversion rate cannot be determined for code {$currency}");
            }
    ...
    public function enrichPromoApiArray(array $result, $deep = false, $identity = null): array
    {
        ...
        $clientGroups = !empty($result['client_groups']) ? (new \FOSSBilling\Core\Utils\Arr())->getPairsForTableByIds('client_group', $this->decodePromoSelection($result['client_groups'])) : null;
    ...
    public function deletePromo(Promo $model): bool
    {
        ...
        if ($this->hasPromoRedemptionHistoryById($promoId)) {
            throw new \FOSSBilling\Core\Exception\InformationException('Promotions with redemption history cannot be deleted. Disable the promotion instead.');
        }
    ...
    public function getProductSetupPrice(Product $product, ?array $config = null): float
    {
        ...
        throw new \FOSSBilling\Core\Exception\BaseException('Unknown period selected for setup price');
    }
    ...
    public function getProductPrice(Product $product, ?array $config = null): float|int|string
    {
        ...
        if ($pp->getType() == ProductPayment::RECURRENT) {
            if (!isset($config['period'])) {
                throw new \FOSSBilling\Core\Exception\BaseException('Product :id payment type is recurrent, but period was not selected', [':id' => $product->getId()]);
            }

            return $this->getEnabledProductPaymentPeriod($pp, (string) $config['period'])->getPrice();
        }

        throw new \FOSSBilling\Core\Exception\BaseException('Unknown Period selected for price');
    }
    ...
    private function getEnabledProductPaymentPeriod(ProductPayment $pp, string $code): ProductPaymentPeriod
    {
        ...
        try {
            ...
            throw new \FOSSBilling\Core\Exception\InformationException('Selected billing period is not available for this product');
        }
    ...
    {
        if (!$productPayment instanceof ProductPayment) {
            throw new \FOSSBilling\Core\Exception\InformationException('Product payment not found');
        }
    ...
    {
        $slug = \FOSSBilling\Core\Utils\Str::slug($title);

        while ($this->getProductRepository()->findOneBy(['slug' => $slug]) instanceof Product) {
            $slug = \FOSSBilling\Core\Utils\Str::slug($title) . '-' . random_int(1, 9999);
        }
        ...
        if ($pricing['type'] == ProductPayment::RECURRENT) {
            ...
            foreach ($submitted as $rawCode => $periodPricing) {
                ...
                try {
                    ...
                    throw new \FOSSBilling\Core\Exception\InformationException('Invalid billing period :period', [':period' => (string) $rawCode]);
                }
            ...
            if ($submittedCodes === []) {
                throw new \FOSSBilling\Core\Exception\InformationException('At least one billing period must be configured for a recurring product');
            }
    ...
    public function getProductModuleService(Product $product): object
    {
        $type = $product->getType();
        if ($type === null || $type === '') {
            throw new \FOSSBilling\Core\Exception\BaseException('Product type could not be determined.');
        }
    ...
    private function getDomainTldModel(array $config): Tld
    {
        ...
        if (!isset($config['action'])) {
            throw new \FOSSBilling\Core\Exception\BaseException('Could not determine domain price. Domain action is missing', null, 498);
        }
        ...
        if (!$tld instanceof Tld) {
            throw new \FOSSBilling\Core\Exception\BaseException('Unknown TLD. Could not determine registration price');
        }
    ...
    private function getFreeDomainDiscountYears(array $addon, string $period): int
    {
        ...
        $freeDomainPeriods = $addon['config']['free_domain_periods'] ?? [];
        if (\FOSSBilling\Core\Utils\Arr::safeCount($freeDomainPeriods) > 0) {
