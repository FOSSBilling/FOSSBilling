...
class Service implements InjectionAwareInterface
{
    ...
    public function action_activate(Order $order): bool
    {
        ...
        if (!is_object($plugin)) {
            throw new \FOSSBilling\Core\Exception\BaseException('License plugin :plugin was not found.', [':plugin' => $model->getPlugin()]);
        }

        if (!method_exists($plugin, 'generate')) {
            throw new \FOSSBilling\Core\Exception\BaseException('License plugin does not have generate method');
        }
        ...
        do {
            ...
            if ($i++ >= $iterations) {
                throw new \FOSSBilling\Core\Exception\BaseException('Maximum number of iterations reached while generating license key');
            }
    ...
    private function _getOrderService(Order $order, bool $required = true): ?ServiceLicense
    {
        ...
        if (!$model instanceof ServiceLicense) {
            if ($required) {
                throw new \FOSSBilling\Core\Exception\BaseException('Could not find associated service license');
            }
