...
// Adapter for Namecheap Dmoain registration/Transfer support.
// DNS is defaulted to ClouDns master zone NS servers

class Registrar_Adapter_NamecheapClouDns extends Registrar_AdapterAbstract
{
    public $config = array(
        'api-user-id' => null,
        'api-key' => null,
    ...
    public function __construct($options)
    {
        ...
        if (isset($options['api-user-id']) && !empty($options['api-user-id'])) {
            ...
            throw new Registrar_Exception('Domain registrar "NamecheapClouDns" is not configured properly. Please update configuration parameter "Reseller ID" at "Configuration -> Domain registration".');
        }
        ...
        if (isset($options['api-key']) && !empty($options['api-key'])) {
            ...
            throw new Registrar_Exception('Domain registrar "NamecheapClouDns" is not configured properly. Please update configuration parameter "API Key" at "Configuration -> Domain registration".');
        }
        ...
        if (isset($options['username']) && !empty($options['username'])) {
            ...
            throw new Registrar_Exception('Domain registrar "NamecheapClouDns" is not configured properly. Please update configuration parameter "Username" at "Configuration -> Domain registration".');
        }
        ...
        if (isset($options['ip']) && !empty($options['ip'])) {
            ...
            throw new Registrar_Exception('Domain registrar "NamecheapClouDns" is not configured properly. Please update configuration parameter "Server IP Address" at "Configuration -> Domain registration".');
        }

        if (isset($options['clouDns-id']) && !empty($options['clouDns-id'])) {
            $this->config['clouDns-id'] = $options['clouDns-id'];
            unset($options['clouDns-id']);
        } else {
            throw new Registrar_Exception('Domain registrar "NamecheapClouDns" is not configured properly. Please update configuration parameter "Server IP Address" at "Configuration -> Domain registration".');
        }

        if (isset($options['clouDns-password']) && !empty($options['clouDns-password'])) {
            $this->config['clouDns-password'] = $options['clouDns-password'];
            unset($options['clouDns-password']);
        } else {
            throw new Registrar_Exception('Domain registrar "NamecheapClouDns" is not configured properly. Please update configuration parameter "Server IP Address" at "Configuration -> Domain registration".');
        }
    }
            ...
            'form'  => array(
                ...
                'clouDns-id' => array(
                    'text', array(
                        ...
                        'description' => 'Login to your ClouDns Account. Go to Reseller API. The table API users indicates all your API users',
                        'required' => true,
                    ),
                ),
                'clouDns-password' => array(
                    'text', array(
                        'label' => 'ClouDns Api user Password',
                        'description' => 'This is the password associated with the ClouDns user entered above',
                        'required' => true,
                    ),
                ),
            ),
     ...
     */
    protected function _makeRequest($params = array(), $method = 'GET', $customUrl = null)
    {
        ...
        $opts = array(
            ...
            CURLOPT_TIMEOUT         => 60,
            CURLOPT_URL             => $customUrl ?? $this->_getApiUrl(),
        ...
        if (isset($result['status']) && $result['status'] == 'error') {
            throw new Registrar_Exception($result['status'], 102);
        }

        if (isset($result['status']) && $result['status'] == 'Failed') {
            throw new Registrar_Exception($result['status'], 103);
        }
    ...
    public function modifyNs(Registrar_Domain $domain)
    {
        ...
        if (!isset($result->CommandResponse->DomainDNSSetCustomResult['Updated']) && $result->CommandResponse->DomainDNSSetCustomResult['Updated'] != 'true') {
            throw new Registrar_Exception('Could not update NameServers');
        }
        return True;
    }
    ...
    {
        $nameservers = $this->clouDnsRegisterDomainZone($domain);
        $params = array(
        ...
        if (isset($result->CommandResponse->DomainTransferCreateResult['Transfer']) && $result->CommandResponse->DomainTransferCreateResult['Transfer'] == 'true') {
            ...
            $this->setNsToClouDns($domain, $nameservers);
            return True;
        }
        error_log('DOMAIN TRANSFER: false');
        throw new Registrar_Exception("Domain transfer could not be completed. Please contact your administrator");
    }
    ...
    public function getDomainDetails(Registrar_Domain $domain)
    {
        ...
        if (!isset($result->CommandResponse->DomainContactsResult)) {
            throw new Registrar_Exception("Could not retrieve domain details");
        }
    ...
    /**
     * This feature is unavailable through namecheaps API
     * @return string
     * @throws Registrar_Exception
     */
    public function getEpp(Registrar_Domain $domain)
    {
        throw new Registrar_Exception('This feature is unavailable through the Namecheap API. Please contact your administrator');
    }
    ...
    public function registerDomain(Registrar_Domain $domain)
    {
        ...
        if ($domain->getNs1()) {
            ...
            $params['Nameservers'] = $this->clouDnsRegisterDomainZone($domain);
        }
    ...
    private function getPrivacyInfo(Registrar_Domain $domain)
    {
        ...
        throw new Registrar_Exception('Could not retrieve privacy info on this domain');
    }
    ...
    public function getLock(Registrar_Domain $domain)
    {
        ...
        if (!isset($result->CommandResponse->DomainGetRegistrarLockResult['RegistrarLockStatus'])) {
            throw new Registrar_Exception('API ERROR: could not get lock status on domain');
        }
    ...
    public function lock(Registrar_Domain $domain)
    {
        ...
        if (!isset($result->CommandResponse->DomainSetRegistrarLockResult['IsSuccess'])) {
            throw new Registrar_Exception('API ERROR: could not set lock status on domain');
        }
     ...
     * @throws Registrar_Exception
     */
    private function clouDnsRegisterDomainZone(Registrar_Domain $domain)
    {
        $url = 'https://api.cloudns.net/dns/register.xml';

        $params = array(
            'auth-id' => $this->config['clouDns-id'],
            'auth-password' => $this->config['clouDns-password'],
            'domain-name' => $domain->getName(),
            'zone-type' => 'master',
        );

        $result = $this->_makeRequest($params, 'POST', $url);

        if (!(string)$result->status == 'Success') {
            throw new Registrar_Exception('Could not create zone at ClouDns');
        }

        error_log('CLOUDNS REGISTER RESULT: ' . $result);

        $url = 'https://api.cloudns.net/dns/records.xml';
        ...
        $result = $this->_makeRequest($params, 'GET', $url);
     ...
     * @throws Registrar_Exception
     */
    private function setNsToClouDns(Registrar_Domain $domain, String $records)
    {
        // get NS info
        $params = array(
            'SLD' => $domain->getSld(),
        ...
        if (!isset($result->CommandResponse->DomainDNSSetCustomResult['Updated']) && $result->CommandResponse->DomainDNSSetCustomResult['Updated'] != 'true') {
            throw new Registrar_Exception('Could create DNS records. Please contact your administrator');
        }
        return True;
    }
}
