<?php
/**
 * Copyright 2022-2024 eNom Domain Registar Adapter for FOSSBilling
...
class Registrar_Adapter_eNom extends Registrar_AdapterAbstract
{
    ...
    public function registerDomain(Registrar_Domain $domain)
    {
        ...
        foreach ($contactTypes as $contactType) {
            ...
            if (method_exists($domain, $contactMethod)) {
                ...
                $contactType = 'Registrar' == $contactType ? 'Registrant' : $contactType;
    ...
    public function isDomainAvailable(Registrar_Domain $domain)
    {
        ...
        try {
            ...
            return 210 == $result->RRPCode;  // Domain name available
        } catch (Registrar_Exception $e) {
    ...
    public function isDomainCanBeTransferred(Registrar_Domain $domain)
    {
        ...
        try {
            ...
            if (210 == $rrpCode) {
                throw new Registrar_Exception('This domain is not registered');
            } elseif (211 != $rrpCode) {
                throw new Registrar_Exception('RRPCode: '.$rrpCode);
            }

            // make sure the domain can be transfered
            if (!$result->Domains->Domain->Properties->Transferable) {
                throw new Registrar_Exception('This domain is not transferable');
            }
    ...
    public function modifyNs(Registrar_Domain $domain)
    {
        ...
        try {
            ...
            return 200 == $result->RRPCode;  // Nameserver modification successful
        } catch (Registrar_Exception $e) {
    ...
    public function enablePrivacyProtection(Registrar_Domain $domain)
    {
        ...
        if ('0' == $wppsInfoResult->GetWPPSInfo->WPPSEnabled) {
            $enableParams = [
            ...
            if (isset($serviceStatus) && 'Enabled' == $serviceStatus) {
                return true;
            } else {
    ...
    public function disablePrivacyProtection(Registrar_Domain $domain)
    {
        ...
        if (isset($wppsInfoResult->GetWPPSInfo->WPPSExists) && '1' == $wppsInfoResult->GetWPPSInfo->WPPSExists) {
            // check if wpps is enabled
            if (isset($wppsInfoResult->GetWPPSInfo->WPPSEnabled) && '1' == $wppsInfoResult->GetWPPSInfo->WPPSEnabled) {
                // disable wpps
                $disableParams = [
                ...
                if (isset($serviceStatus) && 'Disabled' == $serviceStatus) {
                    return true;
                } else {
    ...
    private function fetchPrivacyStatus(Registrar_Domain $domain)
    {
        ...
        if (isset($result->GetWPPSInfo->WPPSEnabled)) {
            return '1' == $result->GetWPPSInfo->WPPSEnabled;
        }
    ...
    private function transferCondition_domainNotFound(Registrar_Domain $domain)
    {
        ...
        if (isset($xml->ErrCount) && 0 == (int) $xml->ErrCount) {
            // domain found in the current registrar
            throw new Registrar_Exception('Domain exist in the current Registrar');
        } elseif (isset($xml->ErrCount) && 1 == (int) $xml->ErrCount) {
            $errors = $xml->errors;
            $errorKey = 'Err1';
            $errMessage = (string) $errors->$errorKey;
            // domain not found
            if ('The remote server returned an error: (404) Not Found.' == $errMessage) {
                return;
            }
            // unknown error
