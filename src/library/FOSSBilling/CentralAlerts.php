...
class CentralAlerts implements InjectionAwareInterface
{
    ...
    public function makeRequest(string $endpoint, array $params = []): array
    {
        ...
        try {
            ...
            throw new \FOSSBilling\Exception('Unable to fetch alerts from Central Alerts System. See error log for more information.', null);
        }

        if (isset($json['error']) && is_array($json['error'])) {
            throw new \FOSSBilling\Exception($json['error']['message'], null);
        }

        if (!isset($json['result']) || !is_array($json['result'])) {
            throw new \FOSSBilling\Exception('Invalid response from the FOSSBilling Central Alerts System.', null);
        }
