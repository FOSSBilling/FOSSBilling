    ...
    private function canUseSession(): void
    {
        ...
        if (Config::getProperty('security.perform_session_fingerprinting', true)) {
            ...
            if (!is_array($storedFingerprint) || !$fingerprint->checkFingerprint($storedFingerprint)) {
                $invalid = true;
                $this->di['logger']->setChannel('security')->warning("Session ID $sessionID has potentially been hijacked as it failed the fingerprint check. The session has automatically been destroyed.");
            }
        }
