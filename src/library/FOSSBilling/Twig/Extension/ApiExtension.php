...
class ApiExtension
{
    ...
    {
        return 'data-fb-api="' . $this->encodeAttributeJson($config) . '"';
    }
    ...
    public function fbApiForm(array $config = []): string
    {
        ...
        $config['type'] = 'form';
        $attr = 'method="post" data-fb-api="' . $this->encodeAttributeJson($config) . '"';
    ...
    private function encodeAttributeJson(array $config): string
    {
        $json = json_encode($config, JSON_THROW_ON_ERROR | JSON_HEX_APOS | JSON_HEX_AMP | JSON_HEX_QUOT | JSON_HEX_TAG | JSON_UNESCAPED_SLASHES);
