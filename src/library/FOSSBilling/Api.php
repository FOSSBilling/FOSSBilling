...
class Api implements InjectionAwareInterface
{
    ...
    {
        $exc = new Exception('Unknown API call :call', [':call' => $page], 879);
    ...
    private function validateRequest($role, $method)
    {
        ...
        if (!empty($allowedIps) && !in_array($clientIp, $allowedIps)) {
            throw new InformationException('Unauthorized IP', null, 1002);
        }
        ...
        if (!in_array($role, $roleAllowed)) {
            throw new InformationException('Unknown API call :call', [':call' => $role], 701);
        }
    }
    ...
    private function checkHttpReferer()
    {
        ...
        if ($check_referer_header) {
            ...
            if (!$referer || !str_starts_with($referer, $url)) {
                throw new InformationException('Invalid request. Make sure request origin is :from', [':from' => SYSTEM_URL], 1004);
            }
        }
    ...
    private function getAuth()
    {
        ...
        if (!isset($_SERVER['PHP_AUTH_USER'])) {
            throw new InformationException('Authentication Failed', null, 201);
        }
        if (!isset($_SERVER['PHP_AUTH_PW'])) {
            throw new InformationException('Authentication Failed', null, 202);
        }
        if (empty($_SERVER['PHP_AUTH_PW'])) {
            throw new InformationException('Authentication Failed', null, 206);
        }
        ...
        switch ($username) {
            ...
            case 'admin':
                ...
                if (!$model instanceof \Model_Admin) {
                    throw new InformationException('Authentication Failed', null, 205);
                }
                $sessionAdminArray = [
                    'id' => $model->id,
                    'email' => $model->email,
            ...
            case 'guest':
            default:
                throw new InformationException('Authentication Failed', null, 203);
        }
    }
    ...
    public function renderJson($data = null, ?\Exception $e = null)
    {
        ...
        header('X-FOSSBilling-Version: ' . Version::VERSION);
