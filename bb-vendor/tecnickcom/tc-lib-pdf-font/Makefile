...
bintray: rpm deb rpm_fonts deb_fonts
	...
	$(foreach FONT,$(FONTLIST), \
		@curl -T target/RPM_FONTS/${FONT}/RPMS/noarch/php-tecnickcom-${PROJECT}-data-${FONT}-${VERSION}-${RELEASE}.noarch.rpm -u${APIUSER}:${APIKEY} -H "X-Bintray-Package:${PROJECT}-data-${FONT}" -H "X-Bintray-Version:${VERSION}" -H "X-Bintray-Publish:1" -H "X-Bintray-Override:1" https://api.bintray.com/content/tecnickcom/rpm/php-tecnickcom-${PROJECT}-data-${FONT}-${VERSION}-${RELEASE}.noarch.rpm && \
		@curl -T target/DEB_FONTS/${FONT}/php-tecnickcom-${PROJECT}-data-${FONT}_${VERSION}-${RELEASE}_all.deb -u${APIUSER}:${APIKEY} -H "X-Bintray-Package:${PROJECT}-data-${FONT}" -H "X-Bintray-Version:${VERSION}" -H "X-Bintray-Debian-Distribution:all" -H "X-Bintray-Debian-Component:main" -H "X-Bintray-Debian-Architecture:all" -H "X-Bintray-Publish:1" -H "X-Bintray-Override:1" https://api.bintray.com/content/tecnickcom/deb/php-tecnickcom-${PROJECT}-data-${FONT}_${VERSION}-${RELEASE}_all.deb ; \
	)
