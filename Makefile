...
install: start ## Install app after start
	docker run --rm --interactive --tty --volume $(PWD):/app composer install --no-progress --prefer-dist --no-dev
	rm -rf ./src/install
...
test: start ## Run app tests
	...
	rm -rf src/install
	docker run --rm --interactive --tty --volume $(PWD):/app composer install --no-progress --no-suggest --prefer-dist
